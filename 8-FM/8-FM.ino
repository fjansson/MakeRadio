...
void setup() 
{
  ...
  // rx.setFmDeemphasis(1);      // standard in Europe
...
{
  char str[20];
  // optionally wait for a button press
  //while(!(digitalRead(gp_seek_down) || digitalRead(gp_seek_up)))
  //  delay(5);
  ...
  { // fetch information and display on screen
    ...
    snprintf(str, 18, "%6.2f MHz  %d", f, rssi);
    lcd.clear();
    lcd.print(str);
    if (stereo)
      lcd.print(" S");
